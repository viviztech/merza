            ...
            @if($product->is_preorder)
                ...
                <section aria-labelledby="prebooking-highlights" class="mb-5 overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm">
                    ...
                    <dl class="grid grid-cols-2 divide-x divide-y divide-emerald-100 sm:grid-cols-2">
                        ...
                        @if($product->farm_location)
                            <div class="min-w-0 p-3.5">
                                <dt class="text-[10px] font-bold uppercase tracking-wider text-stone-400">Grown at</dt>
                                <dd class="mt-1 text-sm font-extrabold leading-snug text-stone-800">{{ $product->farm_location }}</dd>
                            </div>
                        @endif

                        @if($product->sweetness_level)
                            <div class="min-w-0 p-3.5">
                                <dt class="text-[10px] font-bold uppercase tracking-wider text-stone-400">Taste</dt>
                                <dd class="mt-1 text-sm font-extrabold text-stone-800">{{ $product->sweetness_level }}</dd>
                            </div>
                        @endif
                ...
                <div class="mb-5 flex flex-wrap gap-2">
                    <button wire:click="buyNow"
                            ...
                            class="inline-flex min-h-9 items-center justify-center rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-4 py-2 text-sm font-extrabold text-white shadow-sm transition-all hover:from-amber-400 hover:to-orange-400 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50">
                        <span wire:loading.remove wire:target="buyNow">Pre-book now</span>
                        <span wire:loading wire:target="buyNow">Opening…</span>
                    </button>

                    <button wire:click="addToCart"
                            wire:loading.attr="disabled"
                            @if($selectedVariant?->stock_qty <= 0) disabled @endif
                            class="inline-flex min-h-9 items-center justify-center rounded-xl border border-amber-500 bg-white px-4 py-2 text-sm font-extrabold text-amber-700 transition-colors hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-50">
                        <span wire:loading.remove wire:target="addToCart">Add to cart</span>
                        <span wire:loading wire:target="addToCart">Adding…</span>
                    </button>

                    <a href="https://wa.me/918667696278?text=Hi%2C+I+want+to+pre-book+{{ urlencode($product->name) }}"
                       target="_blank"
                       rel="noopener noreferrer"
                       class="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-extrabold text-white transition-colors hover:bg-emerald-700">
