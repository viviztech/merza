<x-layouts.storefront title="Order #{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}">
    <div class="max-w-2xl mx-auto px-4 py-10">
            ...
            <div class="divide-y divide-stone-50">
                @foreach($order->items as $item)
                    <div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                        ...
                        <p class="text-sm font-bold text-stone-800">₹{{ number_format($item->price * $item->quantity, 2) }}</p>
                    </div>
                @endforeach
            </div>
        ...
        <div class="bg-amber-50 border border-amber-100 rounded-2xl p-5 flex items-center justify-between">
            ...
            <a href="https://wa.me/918667696278?text=Hi%2C+I+need+help+with+order+%23{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}"
               target="_blank"
